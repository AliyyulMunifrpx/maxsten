...
describe("calculateStoreStatus Helper", () => {
  // 2026-07-23 05:00:00Z == Asia/Jakarta 12:00 siang, hari Kamis (day=4)
  const JAKARTA_NOON_UTC = "2026-07-23T05:00:00.000Z";

  afterEach(() => {
    // Pastikan timer dikembalikan ke normal setiap test selesai
    vi.useRealTimers();
  });

  describe("Basic Schedule Calculation", () => {
  ...
  describe("Manual Override (is_open) Logic", () => {
    ...
    test("falls back to schedule when manual_status has an unrecognized value", () => {
      ...
      // Nilai selain "OPEN"/"CLOSED" (misal data korup/typo) tidak boleh dianggap "tutup"
      const store = makeStore({
        manual_status: "MAYBE",
        manual_updated_at: now,
      });
  ...
  describe("Timezone & Overnight Shifts Edge Cases", () => {
    ...
    test("overnight schedule correctly reports closed during the daytime gap", () => {
      vi.useFakeTimers();
      // Jakarta 10:00 -- di luar sesi overnight manapun
