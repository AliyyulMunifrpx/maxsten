...
const updateOperationalHoursValidation = Joi.object({
  operational_hours: Joi.array().items(
    Joi.object({
      ...
      open_time: Joi.string().pattern(/^([01]\d|2[0-3]):?([0-5]\d)$/).allow(null, "").optional(), // Format HH:mm
      close_time: Joi.string().pattern(/^([01]\d|2[0-3]):?([0-5]\d)$/).allow(null, "").optional(),
      is_active: Joi.boolean().default(false)
    })
  ).min(1).required() // Minimal kirim 1 hari, maksimal 7
});
export {
