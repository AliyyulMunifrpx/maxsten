...
export default function CatalogPageLoading() {
  return (
    <div className="w-full min-h-full flex flex-col p-[16px] bg-[#1e1e1e] gap-[16px]">
      {/* skeleton header toko */}
      <div className="relative w-full h-[112px] shrink-0 overflow-hidden bg-white/5 border-none">
        <Shimmer />
      </div>

      {/* skeleton search bar */}
      <div className="relative w-full h-[44px] shrink-0 overflow-hidden bg-white/5 border-none">
        <Shimmer />
      </div>

      <div className="w-full grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-[16px]">
        {[...Array(10)].map((_, i) => (
          <div
            key={i}
            className={`aspect-[3/4] ${i >= 6 ? "hidden lg:block" : ""} ${i >= 8 ? "lg:hidden xl:block" : ""}`}
          >
            <ProductCard />
          </div>
        ))}
