...
function MetricCard() {
  return (
    <div className="group relative w-full h-[128px] flex flex-col justify-center bg-white/5 border-none16px] overflow-hidden cursor-pointer">
      <Shimmer />
    </div>
  );
}
...
export default function AnalytictPageLoading() {
  return (
    <div className="w-full flex flex-col p-[16px] h-full bg-[#1e1e1e] gap-[16px]">
      ...
      <div className="flex gap-[16px] grid grid-cols-3 lg:grid-cols-6 w-full">
        {[...Array(6)].map((_, i) => (
          <MetricCard></MetricCard>
        ))}
      </div>
      <div className="w-full h-full grid grid-cols-1 lg:grid-cols-2 flex gap-[16px]">
       <PanelCard></PanelCard>
       <PanelCard></PanelCard>
      </div>
    </div>
